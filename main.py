...
def encode(images):
    encode_list = []
    for img in images:
        # Convert the image to RGB format (required for face_recognition library)
...
def record_attendance(name):
    with open('attendancelist.csv', 'r+') as file:
        ...
        for line in List:
            # Split each line in the attendance file to extract names
            record = line.split(',')
...
# Encode all the images in the 'Images' directory
...
while True:
    ...
    face = face_recognition.face_locations(imgResize, model="hog")  # Use the "cnn" model for accuracy
    ...
    if len(face) > 0:
        ...
        for encodecurr, loc in zip(encodeImg, face):
            match = face_recognition.compare_faces(encodeList, encodecurr, tolerance=1)  # Adjust tolerance here
            ...
            if len(faceDist) > 0:
                ...
                if match[index_BestMatch]:
                    ...
                    # Record the attendance when a match is found
